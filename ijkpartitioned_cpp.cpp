...
int main(int argc, char** argv)
{
     ...
     */
    INTEGER4 dim[3];
    dim[0] = 2; dim[1] = 2; dim[2] = 1;
    INTEGER4 nx = 0, ny = 0, nz = 0;

    INTEGER4 local_nx = XDIM / dim[0];               // X range in a processor
    INTEGER4 local_ny = YDIM / dim[1];
    INTEGER4 local_nz = ZDIM / dim[2];
    INTEGER4 xmin = 1, ymin = 1, zmin = 1, xmax = 1, ymax = 1, zmax = 1;
    INTEGER4 coord[3] = { 0,0,0 };
    /*int myrank = commRank;

    while (myrank != 0) {
        if (myrank > (dim[0] * dim[1])) {
            nz = myrank / (dim[0] * dim[1]);
            myrank = myrank % (nz * (dim[0] * dim[1]));
        }
        else if (myrank > (dim[0])) {
            ny = myrank / (dim[0]);
            myrank = myrank % (ny * (dim[0]));
        }
        else {
            nx = myrank;
            myrank = 0;
    ...
    //INTEGER4 partitionIndices[numProcesses][6];
